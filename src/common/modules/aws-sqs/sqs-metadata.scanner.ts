...
export class SqsMetadataScanner implements OnModuleInit {
  ...
  public async onModuleInit(): Promise<void> {
    const processes =
      await this.discover.providersWithMetaAtKey<SqsProcessMeta>(SQS_PROCESS);
      ...
      processes.map(
        (process) =>
          new Promise((resolve) => {
            ...
            const messageHandlerMetadatas =
              this.discover.classMethodsWithMetaAtKey<SqsMessageHandlerMeta>(
                discoveredClass,
                SQS_CONSUMER_METHOD,
              );
            ...
            const eventHandlerMetadatas =
              this.discover.classMethodsWithMetaAtKey<SqsConsumerEventHandlerMeta>(
                discoveredClass,
                SQS_CONSUMER_EVENT_HANDLER,
              );
            ...
            const sqsMetadata: SqsMetadata = {
              ...
              messageHandler: {
                batch,
                handleMessage: messageMethod.handler.bind(
                  messageMethod.parentClass.instance,
                ),
              },
              eventHandler: eventHandlerMetadatas.map((metadata) => ({
                eventName: metadata.meta.eventName,
                handleEvent: metadata.discoveredMethod.handler.bind(
